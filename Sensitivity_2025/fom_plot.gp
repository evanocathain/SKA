...
unset key
set ylabel "1.4-GHz PSR Survey Speed FoM (arb. units)" font 'Times, 20'
...
#set label at 510,5000 textcolor rgb 'red' "SKA1-Mid PSR Search Radius" font "Times, 20"
...
"<awk '{print $0,sqrt($3*$3+$4*$4)}' ../Configuration_2024/MID_dist_metres.txt | sort -g -k5 | awk -v g=0.0 -v mkgain=6.34 -v skagain=10.16 '{if (substr($2,1,1)==\"M\") g+=mkgain; if (substr($2,1,1)==\"S\") g+=skagain; fov=1.0/(7.5*7.5); if (1500/($5*$5) < fov) fov=1500/($5*$5); print $5,g*g*fov}'" w li lt -1, \
"<awk '{print $0,sqrt($3*$3+$4*$4)}' ../Configuration_2024/MID_AAstar_dist_metres.txt | sort -g -k5 | awk -v g=0.0 -v mkgain=6.34 -v skagain=10.16 '{if (substr($2,1,1)==\"M\") g+=mkgain; if (substr($2,1,1)==\"S\") g+=skagain; fov=1.0/(7.5*7.5); if (1125/($5*$5) < fov) fov=1125/($5*$5); print $5,g*g*fov}'" w li lt -1, \
"<awk '{print $0,sqrt($3*$3+$4*$4)}' ../Configuration_2024/MID_dist_metres.txt | sort -g -k5 | awk -v g=0.0 -v mkgain=6.34 '{if (substr($2,1,1)==\"M\") {g+=mkgain; fov=1.0/(6.75*6.75); if (800/($5*$5) < fov) fov=800/($5*$5); print $5,g*g*fov}}'" w li lt 2, \
...
set label "FAST MB-19" front at 220,3500 #font "Times, 15"
set label "Arecibo MB-7" front at 100,800 #font "Times, 15"
set label "Effelsberg MB-7" front at 40,210 #font "Times, 15"
set label "Parkes MB-13" front at 22,110 #font "Times, 15"

# Label the SKA-Mid components
set label "SKA1-Mid AA4 (1500 beams)" front at 700,3000 #font "Times, 15"
set label "SKA1-Mid AA* (1150 beams)" front at 210,1400 #font "Times, 15"
set label "MeerKAT (800 beams)" front at 800,100 textcolor lt 2 #font "Times, 15"
replot
